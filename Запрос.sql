...
-- Разбор запроса:
SELECT a.Date, a.Source, a.Campaign, a.Ad, a.Click, a.Cost, b.install, b.purchase -- Отбирал необходимые колонки. Они будут отбираться из двух запросов: a и b.
FROM (
	SELECT Date, 'A' AS Source, Campaign, Ad, SUM(Click) AS Click, SUM(Cost) AS Cost
	FROM "таблица 1"
	GROUP BY Date, Source, Campaign, Ad
	...
	*/
WHERE DATE_TRUNC('month', a.Date) = DATE_TRUNC('month', CURRENT_DATE) -- В WHERE я отфильтровал полученные данные, чтобы получить необходимые данные за текущий месяц.
